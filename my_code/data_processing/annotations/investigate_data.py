...
gen_tiers = ["R.G.Right.Practice", "R.G.Left.Phrase", "R.G.Left.Phase", "R.G.Right.Phrase", "R.G.Right.Phase",
...
def clean_label(label):
    ...
    if label.find('-') != -1:
        ...
        label_cleaned = [subval.strip() for subval in split]
    elif label.find('/') != -1:
        split = re.split('/', label)
        label_cleaned = [subval.strip() for subval in split]
    elif label.find(',') != -1:
        ...
        label_cleaned =["relative Position"]
    elif label == "Shape38":
        label_cleaned = ["Shape"]
    else:
        label_cleaned = [label]
