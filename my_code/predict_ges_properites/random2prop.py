...
class Decoder(nn.Module):
    ...
    def weights_init(self, m):
        ...
        if classname.find('Linear') != -1:
            ...
            m.weight.data.fill_(0.0)
            # m.bias.data
            m.bias.data.fill_(-np.log(self.output_dim))
...
class PropPredictor(LightningModule):
    ...
    def accuracy(self, prediction, truth, train=False):
        ...
        if self.hparams.data_feat == "Phase":
            # Always ONE
            rand_sample = torch.ones(prediction.shape)

            # Naive Random
            rand_sample = torch.rand(prediction.shape)

            # Informed Random
            rand_sample = np.random.multinomial(1, [0.0051, 0.1305, 0.4156, 0.1374, 0.3008, 0.0106], size=prediction.shape[0])
            ...
            # Always ONE
            prediction = torch.ones(prediction.shape)

            # Naive Random
            prediction = torch.rand(prediction.shape).round()

	    # Informed Random
            prob = torch.ones(prediction.shape) * 0.487
            prediction = torch.bernoulli(prob)

            # for G_semant
            rand_0 = np.random.binomial(1, 0.0507, size=prediction.shape[0])
            rand_1 = np.random.binomial(1, 0.1292, size=prediction.shape[0])
            rand_2 = np.random.binomial(1, 0.1376, size=prediction.shape[0])
            rand_3 = np.random.binomial(1, 0.0192, size=prediction.shape[0])

            #for G_Phrase
            rand_0 = np.random.binomial(1, 0.2963, size=prediction.shape[0])
            rand_1 = np.random.binomial(1, 0.1574, size=prediction.shape[0])
            rand_2 = np.random.binomial(1, 0.7072, size=prediction.shape[0])
            rand_3 = np.random.binomial(1, 0.1323, size=prediction.shape[0])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 1:
            ...
            #print("\nFiles: ", batch["property"][:, 0])
            #print("Timing: ",  batch["property"][:, 1])

            x = batch["property"][60:90, 1].cpu()
            ...
            for feat in range(self.decoder.output_dim):
                ...
                plt.ylim([-0.01, 1.01])
                plt.plot(x, batch["property"][60:90, feat+2].cpu(), 'r--', x, predicted_prob[60:90, feat].cpu(), 'bs--', markersize=1)
    ...
    def val_dataloader(self):
        ...
        val_subsampler = torch.utils.data.SequentialSampler(self.val_ids)
